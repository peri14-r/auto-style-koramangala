...
await page.locator('.swatch').filter({hasText:'Midnight black'}).click();await page.getByLabel('What do you drive?').fill('Hyundai i20');await page.locator('button[type=submit]').click();
const result={errors,title:await page.title(),draftComplete:await page.evaluate(()=>decodeURIComponent(window.__opened[0]||'').includes('Midnight black')),overflow:await page.evaluate(()=>document.documentElement.scrollWidth-innerWidth)};
